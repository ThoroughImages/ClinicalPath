...
df_time <- df[df$factor == "The average time", ]
p = ggplot(data = df_time, aes(x = variable, y = value)) + geom_boxplot(color =
                                                                          "#4C4948", width = 0.15)
pp = p + stat_summary(
...
pp_point = pp + geom_point(aes(colour = variable, shape = variable), size =
                             3)
ppp = pp_point +
  scale_fill_discrete(breaks = c("Without assistance", "With assistance")) +
  ...
  labs(y = "Time of review per WSI (seconds)\n", x = '') +
  theme(
